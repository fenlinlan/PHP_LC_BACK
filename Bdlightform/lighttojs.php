...
// 	include("comlib.php");		//使用資料庫的呼叫程式
 ...
 include("../Connections/iotcnn.php");		//使用資料庫的呼叫程式
  ...
  //   select * from LC.ledset WHERE 1 order by id asc;
   $qrystr="select * from LC.Ledsitelist WHERE 1 order by id asc;" ;		//將dhtdata的資料找出來
   ...
   while($row = mysqli_fetch_array($result)) 
       {
           $userdata = new data() ; 
           $userdata->id = $row["id"] ;
           $userdata->siteid = $row["siteid"] ;
           $userdata->no = $row["no"] ;
           $userdata->donater = $row["donater"] ;
           $userdata->dbirthday= $row["dbirthday"] ;
           $userdata->dphone= $row["dphone"] ;
           $userdata->dueday = $row["dueday"] ;
